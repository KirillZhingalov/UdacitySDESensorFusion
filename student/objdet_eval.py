...
def measure_detection_performance(detections, labels, labels_valid, min_iou=0.5):
    ...
    for label, valid in zip(labels, labels_valid):
        ...
        if valid: # exclude all labels from statistics which are not considered valid
            ...
            # Detectoin format: cls_idx, x, y, z, h, w, l, yaw

            label_corn = tools.compute_box_corners(label.box.center_x, label.box.center_y, 
                                                   label.box.width, label.box.length, 
                                                   label.box.heading)
            label_box = [
                label.box.center_x - label.box.width / 2., # x1 
                label.box.center_y - label.box.length / 2., # y1
                label.box.center_x + label.box.width / 2., # x2
                label.box.center_y + label.box.width / 2. # y2
            ]
            ## step 2 : loop over all detected objects
            for det in detections:
                ## step 3 : extract the four corners of the current detection
                _, x, y, z, h, w, l, yaw = det
                det_box = [x - w / 2, y - l / 2, x + w / 2, y + l / 2]

                det_corn = tools.compute_box_corners(x, y, w, l, yaw)
                
                ## step 4 : computer the center distance between label and detection bounding-box in x, y, and z
                dist_x = np.linalg.norm(label.box.center_x - x).item()
                dist_y = np.linalg.norm(label.box.center_y - y).item()
                dist_z = np.linalg.norm(label.box.center_z - z).item()

                ## step 5 : compute the intersection over union (IOU) between label and detection bounding-box
                # iou = box_iou(torch.Tensor([label_box]), torch.Tensor([det_box]))[0].item()
                label_corn_poly = Polygon(label_corn)
                det_corn_poly = Polygon(det_corn)
                iou = label_corn_poly.intersection(det_corn_poly).area / label_corn_poly.union(det_corn_poly).area
...
def compute_performance_stats(det_performance_all):
    ...
    tp = np.array(pos_negs)[:, 1].sum()
    fn = np.array(pos_negs)[:, 2].sum()
    fp = np.array(pos_negs)[:, 3].sum()

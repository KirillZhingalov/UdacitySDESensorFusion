...
def create_model(configs):
    ...
    if (configs.arch == 'darknet') and (configs.cfgfile is not None):
        ...
        model = get_pose_net(num_layers=18, 
                             heads=configs.heads, 
    ...
    # load model weights
    model.load_state_dict(torch.load(configs.pretrained_filename, map_location='cpu'), strict=True)
...
def detect_objects(input_bev_maps, model, configs):
    ...
    if len(detections):
        ## step 2 : loop over all detection
        
        # Convert to fpn_resnet output format
        if isinstance(detections, list):
            detections = {det[0]: torch.Tensor([det]) for det in detections}
        
        for cls_idx, cls_det in detections.items():
            if cls_idx != 1 or not cls_det.shape[0]: continue
            
            # Detection format
            # scores, xs, ys, z_coor, dim, direction, clses, yaw
            for det in cls_det:
                ...
                if ((x >= configs.lim_x[0]) and (x <= configs.lim_x[1])
                    ...
                    objects.append([cls_idx, x, y, z, h, w, l, yaw])

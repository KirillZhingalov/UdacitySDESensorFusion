...
configs_det.save_results = False # Save results to file
...
exec_visualization = ['show_bev', 'show_objects_in_bev_labels_in_camera'] # options are 'show_range_image', 'show_bev', 'show_pcl', 'show_labels_in_image', 'show_objects_and_labels_in_bev', 'show_objects_in_bev_labels_in_camera', 'show_tracks', 'show_detection_performance', 'make_tracking_movie'
...
while True:
    try:
        ...
        if lidar_bev.device != torch.device('cpu'):
        ...
        if 'show_pcl' in exec_list:
            print('loaded pcl shape:', lidar_pcl.shape)
            pcl.show_pcl(lidar_pcl, cnt_frame)
